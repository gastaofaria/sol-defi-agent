...
pub fn process_deposit(ctx: Context<Deposit>, amount: u64) -> Result<()> {
    let transfer_cpi_accounts = TransferChecked {
        ...
        to: ctx.accounts.registry_token_account.to_account_info(),
    ...
    let deposit_ratio = amount.checked_div(registry.total_deposits).unwrap();
    let users_shares = registry.total_deposit_shares.checked_mul(deposit_ratio).unwrap();
